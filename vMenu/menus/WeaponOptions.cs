...
namespace vMenuClient.menus
{
    public class WeaponOptions
    {
        ...
        private void CreateMenu()
        {
            ...
            {
                if (!string.IsNullOrEmpty(addonWeapon.Name) && AddonWeapons.Count > 0)
                { 
            ...
            #region Disable submenu if no weapons in that category are allowed.
            if (addonWeaponsMenu.Size == 0)
            {
                addonWeaponsBtn.LeftIcon = MenuItem.Icon.LOCK;
                addonWeaponsBtn.Description = "The server owner removed the permissions for all weapons in this category.";
